...
# hypothesis1 = ['It', 'is', 'a', 'cat', 'at', 'room']
# hypothesis2 = [1,2,3,4,5]
# reference1 = ['It', 'is', 'a', 'cat', 'inside', 'the', 'room']
# reference2 = ['It', 'is', 'a', 'cat', 'at', 'room']
# reference3 = [1,2,3,4,6]
# #there may be several references
# BLEUscore = nltk.translate.bleu_score.sentence_bleu([reference3], hypothesis2)
# print(BLEUscore)
...
def eval_epoch(model, validation_data, device):
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            n_word_total += n_word
            n_word_correct += n_correct
            n_bleu_score += bleu_score
            cnt += 1

    loss_per_word = total_loss/n_word_total
    accuracy = n_word_correct/n_word_total
    n_bleu_score = n_bleu_score/ cnt
...
def train(model, training_data, validation_data, optimizer, device, opt):
    ...
    for epoch_i in range(opt.epoch):
        ...
        start = time.time()
        valid_loss, valid_accu, bleu_score = eval_epoch(model, validation_data, device)
        print('  - (Validation) ppl: {ppl: 8.5f}, accuracy: {accu:3.3f} %, bleu: {bleu: 8.5f}, '\
                'elapse: {elapse:3.3f} min'.format(
                    ppl=math.exp(min(valid_loss, 100)), accu=100*valid_accu, bleu = bleu_score,
                    elapse=(time.time()-start)/60))
